      ...
      <section className="mt-8">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {tradingViewScripts.map((script) => (
            <article key={script.title} className="flex min-h-[13rem] flex-col border border-line bg-panel p-5">
              <div className="flex items-start justify-between gap-4">
                <h3 className="text-lg font-semibold text-ink">{script.title}</h3>
                <span className="shrink-0 border border-line bg-panelSoft px-2.5 py-1 text-xs font-semibold text-muted">
                  {script.status === "public" ? "Public" : "Pending"}
                </span>
              </div>
              <p className="mt-3 text-sm leading-6 text-muted">{script.description}</p>
              {script.href ? (
                <a href={script.href} target="_blank" rel="noreferrer" className="mt-auto w-fit border border-ink bg-ink px-4 py-2 text-sm font-semibold text-white transition hover:bg-panel hover:text-ink">
                  Open script
                </a>
              ) : (
