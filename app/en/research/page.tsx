  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-10 md:px-5 md:py-14">
      <section className="grid gap-8 border-b border-line pb-10 lg:grid-cols-[1fr_0.56fr] lg:items-end">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brass">Quant lab</p>
      ...
      <section className="mt-8 grid gap-5 md:grid-cols-2">
        {researchItems.map((item) => item.external ? (
          <a key={item.href} href={item.href} target="_blank" rel="noreferrer" className="group flex min-h-[13rem] flex-col border border-line bg-panel p-5 transition hover:border-ink">
            <h2 className="text-2xl font-semibold text-ink">{item.title}</h2>
            <p className="mt-3 text-sm leading-6 text-muted">{item.description}</p>
            <span className="mt-auto pt-6 text-sm font-semibold text-ink">Open repository &rarr;</span>
          </a>
        ) : (
          <Link key={item.href} href={item.href} className="group flex min-h-[13rem] flex-col border border-line bg-panel p-5 transition hover:border-ink">
            <h2 className="text-2xl font-semibold text-ink">{item.title}</h2>
            <p className="mt-3 text-sm leading-6 text-muted">{item.description}</p>
            <span className="mt-auto pt-6 text-sm font-semibold text-ink">Explore &rarr;</span>
          </Link>
        ))}
      </section>
    </div>
