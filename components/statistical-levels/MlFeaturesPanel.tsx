...
const featureLabels: Array<[keyof MlFeatureSet, string]> = [
  ["return_1p", "return_1p"],
  ["return_4p", "return_4p"],
  ["return_12p", "return_12p"],
  ["volatility", "volatility"],
  ["drawdown", "drawdown"],
  ["trend_slope", "trend_slope"],
  ["distance_to_long_ma", "distance_to_long_ma"],
  ["extension_zscore", "extension_zscore"],
  ["range_percentile", "range_percentile"],
  ["close_location", "close_location"],
  ["correlation_to_selected_average", "correlation_to_selected_average"],
];
  ...
  return (
    <details className="border border-line bg-panel p-5 md:p-6">
      ...
      <p className="mt-3 max-w-3xl text-sm leading-6 text-muted">
        Estas variables pueden servir como insumos para análisis cuantitativo. No son predicciones ni señales operativas.
      </p>
        ...
        <table className="w-full min-w-[980px] border-collapse text-left text-sm">
          <thead className="text-muted">
            <tr className="border-b border-line">
              ...
              {featureLabels.map(([, label]) => <th key={label} className="py-3 pr-4 font-medium">{label}</th>)}
            </tr>
          </thead>
          <tbody>
            {assets.map((asset) => {
              ...
              return (
                <tr key={asset.ticker} className="border-b border-line/70">
                  ...
                  {featureLabels.map(([key]) => <td key={key} className="py-4 pr-4 text-muted">{formatFeature(features[key])}</td>)}
                </tr>
              );
            })}
          </tbody>
