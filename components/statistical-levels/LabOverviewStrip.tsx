...
export function LabOverviewStrip({ assets, frequency, window }: LabOverviewStripProps) {
  ...
  const items = [
    { label: "Extensión actual", help: "Z-score de distancia frente a la media larga del activo.", value: formatNumber(active?.metric.ma200ExtensionZScore ?? null), width: magnitude(active?.metric.ma200ExtensionZScore ?? null, maxZ), tone: "bg-[#6f8f7b]" },
    { label: "Percentil de extensión", help: "Ubica la extensión actual frente al historial del activo.", value: formatPercentile(active?.metric.ma200ExtensionPercentile ?? null), width: magnitude(active?.metric.ma200ExtensionPercentile ?? null, maxPercentile), tone: "bg-[#b6905b]" },
    { label: "Volatilidad anualizada", help: "Volatilidad anualizada dentro de la ventana seleccionada.", value: formatPercent(active?.metric.annualizedVolatilityWindow ?? null), width: magnitude(active?.metric.annualizedVolatilityWindow ?? null, maxVol), tone: "bg-[#7d8f9a]" },
    { label: "Drawdown actual", help: "Distancia desde el máximo previo dentro de la ventana analizada.", value: formatPercent(active?.metric.currentDrawdown ?? null), width: magnitude(active?.metric.currentDrawdown ?? null, maxDrawdown), tone: "bg-[#9d8176]" },
  ];
