...
const previousPeriod: Record<Period, Period> = {
  "1W": "1M",
  "1M": "3M",
  "3M": "1M",
};
...
function tractionRows(sectors: SectorEtfSnapshot[], period: Period) {
  ...
  return sectors
    .map((sector) => {
      ...
      const priorRank = metricRank(sector, previous);
      const currentReturn = metricValue(sector, period);
      const priorReturn = metricValue(sector, previous);
...
export function SectorRotationChart({ data }: SectorRotationChartProps) {
  ...
  const tractionReference = previousPeriod[period];
    ...
    <section className="border border-line bg-panel p-5 md:p-6">
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.42fr_0.58fr] xl:items-start">
              ...
              return (
                ...
                >
                  ...
                  <div className="relative h-7 bg-panel">
                    ...
                    <div className="absolute left-0 top-1/2 h-px w-full bg-line" aria-hidden="true" />
                    {value !== null ? (
                      <div
        ...
        <aside className="border border-line bg-panelSoft p-4">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-brass">Cambio vs periodo anterior</p>
          <h3 className="mt-2 text-lg font-semibold text-ink">Tracción relativa</h3>
          <p className="mt-2 text-sm leading-6 text-muted">
            Compara {period} contra {tractionReference} para observar quién gana o pierde posición relativa.
          </p>
          ...
          <p className="mt-4 border-t border-line pt-3 text-xs leading-5 text-muted">
            Cambio de tracción basado en ranking y diferencia de retorno entre ventanas. No usa flujos ni entradas/salidas de ETFs.
          </p>
        </aside>
      </div>
