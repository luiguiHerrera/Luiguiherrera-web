import { formatImpliedMove } from "@/lib/reports/report-presentation";
...
function sessionLabel(item: MarketReportEarningsItem) {
  if (item.confirmationStatus !== "confirmed") return "Hora por confirmar";
  return [item.originalTime && item.originalTimeZone ? `${item.originalTime} ${item.originalTimeZone}` : null, item.displayTime].filter(Boolean).join(" · ");
}
