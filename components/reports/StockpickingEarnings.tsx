...
import type { MarketReportEarningsItem, MarketReportUpcomingEarningsItem, MarketReportStockpickingTheme } from "@/lib/reports/market-reports";

function exceeded(item: MarketReportEarningsItem) { return Math.abs(item.actualMovePct ?? 0) > item.impliedMovePct; }

function EvidenceLinks({ item }: { item: MarketReportUpcomingEarningsItem }) {
  if (item.impliedMovePct === undefined) return <p className="text-xs leading-5 text-muted md:col-span-4"><a className="font-semibold text-petrol" href={item.dateTimeSourceHref} target="_blank" rel="noopener noreferrer">{item.dateTimeSourceLabel} ↗</a></p>;
...
export function StockpickingSummary({ published, upcoming }: { published: MarketReportEarningsItem[]; upcoming: MarketReportUpcomingEarningsItem[] }) {
...
export function StockpickingEarnings({ published, upcoming, methodology, publishedNote, upcomingNote, upcomingTitle, themes }: { published: MarketReportEarningsItem[]; upcoming: MarketReportUpcomingEarningsItem[]; methodology: string; publishedNote?: string; upcomingNote?: string; upcomingTitle?: string; themes?: MarketReportStockpickingTheme[] }) {
  return <section className="grid gap-5 pt-2" aria-label="Resultados de Stockpicking">
    ...
    {upcoming.length > 0 ? <div><p className="text-xs font-semibold uppercase text-brass">{upcomingTitle ?? "Qué esperamos"}</p><p className="mt-2 text-sm leading-6 text-muted">{upcomingNote ?? "Nueve ventanas ordenadas cronológicamente; sin una hora confirmada, el evento permanece como día completo."}</p>
      {upcoming.every((item) => item.impliedMovePct === undefined) ? <div className="mt-3 grid gap-3">{upcoming.map((item) => <article key={item.ticker} className="border-l border-brass/35 pl-3 text-sm leading-6">
        <p><strong>{item.company} ({item.ticker})</strong> · {item.reportDate}</p>
        <p className="text-muted">{earningsScheduleLabel(item)}</p>
        <EvidenceLinks item={item} />
      </article>)}</div> : <div className="mt-3 md:border-x md:border-b md:border-line md:px-3"><div className="hidden grid-cols-[1.1fr_0.58fr_0.86fr_0.8fr] gap-2 bg-panelSoft py-2 text-xs font-semibold uppercase text-muted md:grid"><span>Compañía</span><span>Movimiento implícito</span><span>Hora / estado</span><span>Fecha y hora</span></div>{upcoming.map((item) => <div key={item.ticker} className="grid gap-2 border-t border-line py-3 text-sm md:grid-cols-[1.1fr_0.58fr_0.86fr_0.8fr] md:items-center"><div><strong className="text-ink">{item.company}</strong> <span className="text-petrol">({item.ticker})</span><p className="text-xs text-muted">{item.reportDate}</p></div><div><span className="md:hidden">Movimiento implícito esperado: </span><strong>{formatImpliedMove(item)}</strong></div><span className="text-muted">{earningsScheduleLabel(item)}</span><a className="w-fit border-b border-petrol/35 font-semibold text-petrol" href={item.dateTimeSourceHref} target="_blank" rel="noopener noreferrer">Fuente de fecha y hora ↗</a><EvidenceLinks item={item} /></div>)}</div>}
    </div> : upcomingNote ? <p className="text-sm leading-6 text-muted">{upcomingNote}</p> : null}
