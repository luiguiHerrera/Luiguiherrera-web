...
try{
  for(const [locale,name,width,height] of [['es','desktop-primary-es',1440,900],['en','desktop-primary-en',1440,900],['es','large-desktop',1600,900],['es','tablet',768,1024],['es','mobile-primary-es',390,844],['en','mobile-primary-en',390,844]]){
    ...
    for(const id of ['sl-drawdown-history','sl-horizons','sl-quant','sl-assets','sl-guide']){
      ...
      if(id==='sl-drawdown-history'){await open(c,'sl-drawdown-data');assert.equal(await c.evaluate('document.querySelectorAll("#sl-drawdown-data tbody tr").length'),(await readJson('lib/statistical-levels/generated/assets/SPY.json')).frequencies.weekly.windows['5Y'].sessions);await audit(c,name+'/dated-drawdowns');}
  ...
  for(const route of ['/niveles-estadisticos','/en/statistical-levels','/metodologia','/en/methodology']){
    const response=await globalThis.__SL_RELEASE_QA__.protectedGet(base+route),html=await response.text();assert.equal(response.status,200);assert.ok(html.includes('<h1'));report.routes.push({route,status:response.status});
  }
